...
var treeSpecs = [{ desc: 'simplest possible tree',
  data: [100, 100],
  expected: [0, 0]
}, { desc: 'simple tree',
...
var contentSel = select(content);
var button = document.getElementById('spacing');
var options = urlOpts(document.location.href);
var treeFilter = function treeFilter(t) {
  return !('tree' in options) || options.tree === t.i;
};
var results = Array(treeSpecs.length).fill(null);
var nodeSize = function nodeSize(node) {
  return node.data.slice(0, 2);
};
var children = function children(data) {
  var kd = data.slice(2);
...
var minSize = function minSize(xy) {
  return function (tree) {
    return tree.nodes.reduce(function (min, node) {
      return Math.min(min, nodeSize(node)[xy]);
    }, Infinity);
  };
};
var minXSize = minSize(0);
var minYSize = minSize(1);

function displayTree(treeSpec) {
  var layout = flextree({ nodeSize: nodeSize });
  var tree = layout.hierarchy(treeSpec.data, children);
  if (customSpacing) {
    layout.spacing(function (a, b) {
      return a.path(b).length * minXSize(tree) * 0.2;
    });
  }
  var nextId = 0;
...
function setExpectedCoords(node, coords) {
  node.x = coords[0];
  node.y = coords[1];
  (node.children || []).forEach(function (kidNode, i) {
    return setExpectedCoords(kidNode, coords.slice(2)[i]);
  });
  return node;
}
...
function drawTree(label, tree) {
  ...
  var scale = Math.min(width / (extents.right - extents.left), height / extents.bottom);
  var transX = -extents.left * scale;
  var div = contentSel.append('div').attr('class', 'tree');
  div.append('h3').text(label);
  var svgElem = div.append('svg').attrs({
    width: width + 2 * padding,
    height: height + 2 * padding
  });
  var drawing = svgElem.append('g').attr('transform', 'translate(' + (padding + transX) + ' ' + padding + ') scale(' + scale + ' ' + scale + ')');
  var boxPadding = {
    side: minXSize(tree) * 0.1,
    bottom: minYSize(tree) * 0.2
  };
  var context = { drawing: drawing, scale: scale, boxPadding: boxPadding };
  drawSubtree(context, tree);
}

function drawSubtree(context, node) {
  var parent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  var drawing = context.drawing,
      scale = context.scale,
      boxPadding = context.boxPadding;
  ...
  if (parent) {
    drawing.append('path').attr('d', linkVertical()({
      source: [parent.x, parent.y + parent.size[1] - boxPadding.bottom],
...
treeSpecs.forEach(function (treeSpec, num) {
  return treeSpec.num = num;
});
displayAll();
